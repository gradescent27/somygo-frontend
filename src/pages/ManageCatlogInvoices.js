...
import {
  ...
  Mail, Phone, User, DollarSign, CheckCircle,
  XCircle, Clock, AlertCircle, Package, TrendingUp,
  Settings, Zap, Building, Plus, Trash2, Save,
...
const ManageCatlogInvoices = () => {
  ...
  const [filters, setFilters] = useState({ status: 'all', invoiceId: 'all', search: '' });
  ...
  const calculateStats = (data) => {
    ...
    data.forEach(customer => {
      const paidIds = new Set((customer.paidInvoices || []).map(p => p.invoiceId));
      ...
      (customer.paidInvoices || []).forEach(p => {
        ...
        revenueByInvoice[key] = (revenueByInvoice[key] || 0) + (p.productPrice || 0);
      });
  ...
  const handleMarkAttemptAsPaid = async (invoice, attemptIndex) => {
    ...
    const alreadyPaid = invoice.paidInvoices?.some(p => p.invoiceId === attempt.invoiceId);
    ...
    try {
      const response = await apiService.markCatalogInvoiceAsPaid(invoice._id, attemptIndex);
      if (response.success) {
  ...
  const handleMarkLatestAsPaid = async (invoice) => {
    ...
    const alreadyPaid = invoice.paidInvoices?.some(p => p.invoiceId === latest.invoiceId);
  ...
  const fetchSettings = async () => {
    ...
    try {
      const response = await apiService.getSettings(ADMIN_PASSWORD);
      if (response.success) setSettings(response.data);
    } catch (error) {
      console.error('Error fetching settings:', error);
      setSettingsMessage({ type: 'error', text: 'Failed to load settings: ' + error.message });
    } finally {
  ...
  const saveSettings = async (update) => {
    ...
    try {
      const response = await apiService.updateSettings(ADMIN_PASSWORD, update);
      if (response.success) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4">
        ...
        >
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 mt-2">Enter password to access management</p>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-acblue rounded-xl p-5 text-white">
                ...
                <p className="text-3xl font-bold">${stats.totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                <p className="text-xs text-white/50 mt-2">{stats.totalPaid} invoice{stats.totalPaid !== 1 ? 's' : ''} paid</p>
                {Object.keys(stats.revenueByInvoice).length > 0 && (
                  <div className="mt-3 pt-3 border-t border-white/10 flex flex-wrap gap-2">
                    {Object.entries(stats.revenueByInvoice).map(([inv, rev]) => (
                      <span key={inv} className="text-xs bg-white/10 rounded px-2 py-0.5">
              ...
              <div className="bg-white rounded-xl shadow p-5 border-l-4 border-blue-500">
                ...
                <p className="text-3xl font-bold text-blue-600">{stats.awaitingPayment}</p>
                <p className="text-xs text-gray-400 mt-1">Checkout ready, not yet paid</p>
              </div>
              ...
              <div className="bg-white rounded-xl shadow p-5 border-l-4 border-gray-300">
                ...
                <p className="text-xs text-gray-400 mt-1">{stats.totalAttempts} total attempt{stats.totalAttempts !== 1 ? 's' : ''}</p>
              </div>

              {/* Status breakdown: 4 mini cards stacked in a single column */}
              <div className="grid grid-cols-2 md:grid-cols-1 gap-2">
                {[
                  { label: 'Paid', value: stats.totalPaid, icon: CheckCircle, accent: 'border-green-500', valueColor: 'text-green-600' },
                  { label: 'Checkout Ready', value: stats.awaitingPayment, icon: TrendingUp, accent: 'border-blue-500', valueColor: 'text-blue-600' },
                  { label: 'Processing', value: stats.processing, icon: RefreshCw, accent: 'border-yellow-500', valueColor: 'text-yellow-600' },
                  { label: 'Failed', value: stats.automationFailed, icon: XCircle, accent: 'border-red-500', valueColor: 'text-red-600' },
                ].map(({ label, value, icon: Icon, accent, valueColor }) => (
                  <div key={label} className={`bg-white rounded-lg shadow-sm px-3 py-2 border-l-4 ${accent} flex items-center justify-between`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                >
                  ...
                  {settings?.products?.map(p => (
                    <option key={p.invoiceId} value={p.invoiceId}>Invoice #{p.invoiceId}</option>
                  )) || [1,2,3,4].map(i => (
                    <option key={i} value={String(i)}>Invoice #{i}</option>
                  ))}
                </select>
                <button
            ...
            <div className="space-y-4">
              {invoices.map((invoice) => {
                ...
                (invoice.attemptHistory || []).forEach((attempt, idx) => {
                  ...
                  attemptsByInvoice[attempt.invoiceId].push({ ...attempt, _realIdx: idx });
                });
                ...
                return (
                  <div key={invoice._id} className="bg-white rounded-lg shadow overflow-hidden">
                    {/* Customer header */}
                    <div
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            {invoice.originalEmail && invoice.email && invoice.email !== invoice.originalEmail && (
                              <p className="text-xs text-gray-400 flex items-center gap-1 ml-0.5">
                                <span className="text-gray-300">used:</span> {invoice.email}
                              </p>
                            )}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            {invoiceIds.map(invId => {
                              ...
                              const paidInfo = (invoice.paidInvoices || []).find(p => p.invoiceId === invId);
                              const latestAttempt = attemptsByInvoice[invId][attemptsByInvoice[invId].length - 1];
                              const timestamp = isPaid && paidInfo
                                ? new Date(paidInfo.paidAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                                : new Date(latestAttempt.attemptedAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                              return (
                                <div
                                  key={invId}
                                  className={`flex items-start gap-2 px-2.5 py-1.5 rounded-lg text-xs ${
                                    isPaid ? 'bg-green-50 text-green-700' :
                                    latestAttempt.status === 'success' ? 'bg-blue-50 text-blue-700' :
                                    latestAttempt.status === 'failed' ? 'bg-red-50 text-red-700' :
                                    'bg-yellow-50 text-yellow-700'
                                  }`}
                                >
                                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1 ${
                                    isPaid ? 'bg-green-500' :
                                    latestAttempt.status === 'success' ? 'bg-blue-500' :
                                    latestAttempt.status === 'failed' ? 'bg-red-500' :
                                    'bg-yellow-500'
                                  }`} />
                                  <div className="leading-tight">
                                    <p className="font-semibold">Invoice {invId}</p>
                                    <p className="font-normal opacity-60">{timestamp}</p>
                                  </div>
                                </div>
                              );
                            })}
                          ...
                          <div className="text-right text-xs text-gray-500 hidden md:block">
                            ...
                            <p className="text-green-600 font-medium">{invoice.totalPaidInvoices || 0} paid</p>
                          </div>
                        ...
                        >
                          <div className="border-t border-gray-100 px-4 md:px-5 py-4 bg-gray-50 space-y-4">
                            {invoiceIds.map(invId => {
                              ...
                              return (
                                <div key={invId} className={`rounded-lg border ${
                                  isPaid ? 'border-green-200 bg-green-50/50' : 'border-gray-200 bg-white'
                                }`}>
                                  {/* Invoice tile header */}
                                  <div className="p-3 md:p-4 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${
                                        ...
                                        latestAttempt.status === 'success' ? 'bg-blue-500 text-white' :
                                      ...
                                      <div>
                                        <p className="font-medium text-gray-900 text-sm">
                                          Invoice #{invId}
                                          {isPaid && <span className="ml-2 text-green-600 text-xs font-semibold">PAID</span>}
                                        </p>
                                        <p className="text-xs text-gray-500">
                                          {attempts.length} attempt{attempts.length !== 1 ? 's' : ''}
                                          {paidInfo && ` · Paid ${new Date(paidInfo.paidAt).toLocaleDateString()}`}
                                          {paidInfo && ` · $${paidInfo.productPrice}`}
                                        </p>
                                      </div>
                                    </div>
                                    {isPaid ? (
                                      ...
                                      <button
                                        onClick={(e) => { e.stopPropagation(); handleMarkAttemptAsPaid(invoice, latestAttempt._realIdx); }}
                                        className="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors"
                                      >
                                        Mark Paid
                                      </button>
                                    ) : latestAttempt.status === 'failed' ? (
                                      <span className="px-2 py-1 bg-red-100 text-red-600 text-xs font-semibold rounded">Failed</span>
                                    ) : (
                                      <span className="px-2 py-1 bg-yellow-100 text-yellow-700 text-xs font-semibold rounded">Processing</span>
                                    )}
                                  </div>
                                  ...
                                  {attempts.length > 1 && (
                                    <div className="border-t border-gray-100 px-3 md:px-4 py-2 space-y-1">
                                      <p className="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Attempts</p>
                                      {attempts.map((attempt, i) => {
                                        const attemptPaid = isPaid && i === attempts.length - 1;
                                        return (
                                          <div key={i} className="flex items-center justify-between py-1 text-xs">
                                            <div className="flex items-center gap-2 text-gray-500">
                                              <span className={`w-1.5 h-1.5 rounded-full ${
                                                attemptPaid ? 'bg-green-500' :
                                                attempt.status === 'success' ? 'bg-blue-500' :
                                              ...
                                              {attempt.automationType && (
                                                <span className="text-gray-400 bg-gray-100 px-1 py-0.5 rounded">{attempt.automationType}</span>
                                              )}
                                            </div>
                                            <span className={`font-medium ${
                                              attemptPaid ? 'text-green-600' :
                                              attempt.status === 'success' ? 'text-blue-600' :
                                              attempt.status === 'failed' ? 'text-red-500' : 'text-yellow-600'
                                            }`}>
                                              {attemptPaid ? 'paid' : attempt.status === 'success' ? 'checkout ready' : attempt.status}
                                            </span>
                                          </div>
                                        );
                                      })}
          ...
          <div className="space-y-6">
            {settingsLoading ? (
              ...
              <>
                ...
                {settingsMessage && (
                  <div className={`p-4 rounded-lg text-sm font-medium ${
                    settingsMessage.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' :
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    Choose which payment automation runs when a client checks out.
                    Changing this affects all new checkout attempts immediately.
                  </p>
                  <div className="flex flex-wrap gap-3">
                    {(settings.availableAutomations || ['catlog', 'bumpa', 'manual']).map(type => (
                      <button
                        key={type}
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Edit product names, prices, and descriptions. Product names are shown to the payment gateway (intentionally different from descriptions).
                      </p>
                    </div>
                    <button
                      onClick={() => {
                        const newId = String((settings.products?.length || 0) + 1);
                  ...
                  <div className="space-y-3">
                    {(settings.products || []).map((product, idx) => (
                      <div key={idx} className={`border rounded-lg p-4 ${product.enabled ? 'border-gray-200' : 'border-gray-100 bg-gray-50 opacity-60'}`}>
                        <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const updated = [...settings.products];
                                updated[idx] = { ...updated[idx], invoiceId: e.target.value };
                                setSettings(prev => ({ ...prev, products: updated }));
                              }}
                          ...
                          <div className="md:col-span-2">
                            <label className="block text-xs font-medium text-gray-500 mb-1">Product Name (gateway)</label>
                            <input
                              type="text"
                            ...
                            <button
                              onClick={() => {
                                if (window.confirm(`Delete product "${product.productName}"?`)) {
                                  const updated = settings.products.filter((_, i) => i !== idx);
                                  setSettings(prev => ({ ...prev, products: updated }));
                                }
                              }}
                  ...
                  <button
                    onClick={() => saveSettings({ products: settings.products })}
                    disabled={settingsSaving}
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    Displayed on invoice pages. Changes take effect on the next invoice load.
                  </p>
                  ...
                  <button
                    onClick={() => saveSettings({ companyInfo: settings.companyInfo })}
                    disabled={settingsSaving}
                    className="mt-4 flex items-center gap-2 px-4 py-2 bg-acblue hover:bg-blue-900 text-white rounded-lg text-sm font-semibold transition-colors disabled:opacity-50"
                  >
                    <Save className="w-4 h-4" />
              ...
              <div className="text-center py-12">
                ...
                <button onClick={fetchSettings} className="mt-2 text-acblue underline text-sm">Retry</button>
              </div>
            )}
          </div>
        )}
